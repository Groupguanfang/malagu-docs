...
title: 云平台适配
icon: i-ph-cloud
description: Cell.js 框架是平台无关的，可以部署在各种 Serverless 平台、传统服务器之上。Cell.js 框架抽象了一套统一的后端接口，不同平台只需要适配统一的后端接口即可。
---
...
首先，确定我们需要运行和部署的平台，选择合适的适配器组件添加到项目依赖中。比如 `@celljs/express` 、 `@celljs/fc-adapter` 、`@celljs/fc-plugin` 、 `@celljs/scf-adapter` 、`@celljs/scf-plugin` 、`@celljs/lambda-adapter` 、`@celljs/lambda-plugin`、 `@celljs/vercel-adapter` 。Cell.js 组件是一个普通的 npm 包，可以通过以下命令安装：
