---
title: 中间件
icon: i-ph-wave-square
description: Cell.js 中间件与 Koa 里面的中间件是一样的概念，实现了一个洋葱模型，通过中间件可以对请求的处理进行扩展增强，Cell.js 框架本身很多功能就是通过中间件来实现的，比如 Cookies、Session、认证和授权等等。
---

## 接口

中间件必须要实现以下的`Middleware`接口：
...
::note
注意：调用 next 方法的时候一定要记得加`await`关键字，否则会导致全局异常处理失效的问题。
::
