...
title: 请求上下文
icon: i-ph-waves
description: 请求上下文是一个请求级别的共享变量，可以方便不同的业务方法共享请求级别的全局变量，比如 cookie 和 session 会放到请求上下文中，方便用户在自定义的中间件或者业务代码中获取。
---

`请求上下文`与请求相关，每个请求都有属于自己的请求上下文，可以理解为`请求级别的共享变量`就可以存放在`请求上下文`中，这样就可以方便不同的业务方法共享请求级别的全局变量，比如 `cookie` 和 `session` 会放到请求上下文中，方便用户在`自定义的中间件`或者`业务代码`中获取。
