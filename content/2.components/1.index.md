...
description: 这边是Cell.js各个组件的文档，你可以在这里找到你需要的组件的文档。
---
...
to: /components/core
---
查看Cell.js核心组件`@celljs/core`等的文档。
::
...
title: 前端
icon: i-mdi-web
to: /components/frontend
---
结合`React`、`Vue`等前端框架开发应用。
::
...
---
`@celljs/typeorm`等的文档，提供数据库操作相关的能力。
::
...
to: /components/security/central
---
`@celljs/security`的文档，提供认证与授权相关的能力。
::
...
to: /components/cache
---
通过集成`cache-manager`库实现的缓存，让缓存更加简单、与业务代码解耦。
::
...
to: /components/static
---
通过与`serve-static`库结合实现静态资源的访问，让您的应用程序更加完善。
::
...
Cell.js 框架本身也是由一些列组件组成，随着越来越多的组件加入，Cell.js 框架将提供越来越丰富的功能。比如 Cell.js 框架的 `@celljs/core`: 组件，其他组件几乎直接或者间接依赖于它，它提供了框架最为核心的能力，其他组件基于它可以更方便的提供垂直化的能力，如前端开发、Web 服务、微服务等等。

## Cell.js 框架包含的基础组件依赖关系

![依赖关系](/images/deps.webp)
...
- `@celljs/lambda-plugin`: AWS Lambda 插件，集成 AWS Lambda，提供部署、查看等能力
- `@celljs/next-plugin`: Next.js 插件，提供 Next.js 框架项目的构建能力，让 Next.js 也能使用 Cell.js CLI 进行部署，目前支持阿里云函数计算、腾讯云函数
...
- `@celljs/static-plugin`: 静态网站插件，提供静态网站项目通用构建能力，Cell.js 支持 React、Vue、angular 等等前端项目一键部署能力，是通过静态网站插件完成
...
- `@celljs/cli`: 命令行模块，提供 Cell.js CLI 所有能力，提供一个应用生命周期基本都会用到的命令实现与抽象，比如初始化、运行、构建、部署、配置、查看等等。大部份的命令只是抽象，命令的具体有特定的组件提供。比如构建与运行是由 @celljs/cli-service 插件提供
- `@celljs/ext-scripts`: 脚本模块，提供了组件编译、测试、Lint 等通用脚本
- `@celljs/frameworks`: 框架适配模块，Cell.js CLI 不仅可以构建、运行、部署 Cell.js 应用框架，还可以是非 Cell.js 框架的应用。比如 Next.js、Koa.js、Express.js、Nest.js 等数十款框架，未来会持续适配更多框架
