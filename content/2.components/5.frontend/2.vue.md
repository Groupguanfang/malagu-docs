...
description: Cell.js 提供了一套 Vue 开发的能力，你可以无感结合 Vue 开发应用。
---

Cell.js 命令行工具提供了一些列的应用开发模板，我们不需要完全从零开始创建项目。基于模块快速创建项目，然后再根据情况增减相关组件。模板内提供示例代码，让我们快速学习上手。

```bash
$ cell init -o vue-app # 基于 vue-app 模板初始化项目
```

接入 Vue 比较简单，我们没有在Vue上抽离出任何的概念，只需要使用 App 装饰器把 Vue app 实例放进容器中即可。
具体代码如下：
