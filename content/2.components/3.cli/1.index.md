...
title: 介绍
icon: i-carbon-code
description: 了解如何使用Cell.js CLI构建、运行和部署应用。
---

Cell.js 的命令行工具`@celljs/cli`提供了一些基础的命令，能帮助我们`初始化`、`运行`、`构建`和`部署`项目。该工具只包含最核心的功能，通过命令行插件机制把一些非核心的功能、不同平台部署逻辑通过插件的方式提供，命令行插件也是组件的一种形式。通过插件机制，我们可以为命令行工具`添加新的命令`，或者`扩展老的命令`。在 Cell.js 框架中，很多地方都使用到了命令行插件。

命令行工具除了`cell init`命令以外，其他命令都会委托给当前项目中的`@celljs/service`执行，无需担心全局命令与项目本地命令版本不一致问题。命令行工具基于`npm`和`yarn`进行包的管理，会基于当前项目的环境情况智能选择合适的包管理工具，如果`npm`和`yarn`都满足条件，则优先选择使用`yarn`工具。

例如：当项目根目录下存在`yarn.lock`文件，则使用`yarn`。当然，我们也可以强行指定包管理器，在`cell.yml`配置`cell.packager`即可。
...
9. **cell update**： 一键升级 Cell.js cli 和当前项目中 Cell.js 相关依赖的版本
...
- `template`：应用模板，可选，支持指定`内置模板名称`和`GitHub Repository`地址。如果指定`template`参数，则必须指定应用名称；如果没有指定`template`参数，命令行会提示我们从内置模板中选择一个
...
我们可以通过`cell deploy`命令部署项目，使用部署命令时，需要指定部署的平台，比如需要部署到`腾讯云函数`，可以通过`cell deploy -m scf`，部署到`阿里云函数计算`则指定`mode`为`fc`即可，具体可参考[云平台适配]()。

推荐使用`npx cell deploy`部署项目，`cell deploy`命令如下：
...
命令行插件`也是组件的一种形式`。通过插件机制，我们可以为命令行工具`添加新的命令`，或者`扩展老的命令`。在 Cell.js 框架中，很多地方都使用到了命令行 Hook。

<CardGrid>

<LinkCard
  title="CLI插件"
  description="了解如何撰写Cell.js CLI插件。"
  href="/main/cli/hook/#cli-插件"
/>

<LinkCard
  title="Webpack插件"
  description="了解如何撰写Cell.js Webpack插件。"
  href="/maincli/hook/#webpack-插件"
/>

<LinkCard
  title="Deploy插件"
  description="了解如何撰写Cell.js Deploy插件。"
  href="/main/cli/hook/#deploy-插件"
/>

<LinkCard
  title="Build插件"
  description="了解如何撰写Cell.js Build插件。"
  href="/main/cli/hook/#build-插件"
/>

<LinkCard
  title="Serve插件"
  description="了解如何撰写Cell.js Serve插件。"
  href="/main/cli/hook/#serve-插件"
/>

<LinkCard
  title="Config插件"
  description="了解如何撰写Cell.js Config插件。"
  href="/main/cli/hook/#config-插件"
/>

<LinkCard
  title="Info插件"
  description="了解如何撰写Cell.js Info插件。"
  href="/main/cli/hook/#info-插件"
/>

<LinkCard
  title="Init插件"
  description="了解如何撰写Cell.js Init插件。"
