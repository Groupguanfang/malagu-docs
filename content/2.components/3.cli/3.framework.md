...
description: Cell.js 智能框架适配能力本质是基于组件实现。Cell.js 提供了一个框架适配组件@celljs/frameworks。该组件会根据当前项目内容匹配到的适配规则运行个性化地调试、运行、构建、部署等操作。对于主流的开发框架，Cell.js 一般会默认提供适配规则，对于识别到的框架，我们可以使用 Cell.js 命令行工具进行调试、运行、构建、部署等各种 Cell.js 标准命令行操作。
---

Cell.js 命令行是对应用的开发、调试、运行、构建、部署等全生命周期的管理，是一套统一、标准化的研发流程规范。Cell.js 命令行工具不仅仅对 Cell.js 应用框架适用，`NestJS`、`Koa`、`ExpressJS`、`React`、`Vue`、`Vite`、`Angular`、`Svelte`、`H5`、`Preact`、`NextJS`、`NuxtJS`、`UmiJS`、`flet`等等框架都可以使用 Cell.js 命令行工具管理应用的全生命周期。理论上是可以适配任意语言、任意应用开发框架。
...
当我们发现某个好用的框架，Cell.js 并没有适配的时候，我们可以通过 PR 的方式提交到 Cell.js 框架的代码仓库上来。框架的适配规则很简单，我们可以参加 Cell.js 默认提供的适配规则。示例如下：
...
对于一些特殊的框架，我们也可以直接基于`cell.yml`来配置相关的规则。或者我们也可以通过配置命令指定我们自己的框架适配规则源地址。如下所示：
