...
description: Malagu 智能框架适配能力本质是基于组件实现。Malagu 提供了一个框架适配组件@malagu/frameworks。该组件会根据当前项目内容匹配到的适配规则运行个性化地调试、运行、构建、部署等操作。对于主流的开发框架，Malagu 一般会默认提供适配规则，对于识别到的框架，我们可以使用 Malagu 命令行工具进行调试、运行、构建、部署等各种 Malagu 标准命令行操作。
---
