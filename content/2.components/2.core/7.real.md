...
title: 核心原理
icon: i-ph-atom
description: Cell.js 的核心原理，帮助您更好的理解 Cell.js 框架。
---
Cell.js 框架可以开发前端、后端和其他类型应用（比如没有监听任何端口，比如命令行应用）。不管什么样的应用，都会存在一些共性的地方。Cell.js 框架抽象了一个应用程序接口 `Application` 。这个接口足够简单，只有 `start` 和 `stop`方法。不同类型的应用的启动方式可能不同。比如前端应用在启动时，前端 Dom 对象会挂载到 `Document` 上。
...
Cell.js 框架通过命令行工具，外加框架本身的渐进式能力，使我们不用关心框架底层是如何启动的，框架会根据组件依赖情况，自动加载模块，并采用适合的方式启动应用程序，我们只需要使用 `cell serve`  命令即可。在一些特殊的情况下，我们也可以独立启动应用。独立启动应用大体分为两步：
...
下面是关于`@celljs/core`的核心配置项。

### cell.hostDomId

用来定义前端的根元素 id，默认值：cell-root，示例：
...
### cell.annotation.Component.proxy
...
Application 是 Cell.js 应用的入口，以[common/application-entry.ts](https://github.com/cellbang/cell/blob/master/packages/core/src/common/application/application-entry.ts)为例：
...
start 方法中调用了`this._shell.attach(host)`来处理前端的渲染，`this._shell`是`ApplicationShell`的实例，`ApplicationShell`在[browser/shell/application-shell.ts](https://github.com/cellbang/cell/blob/master/packages/core/src/browser/shell/application-shell.ts)文件中的实现如下：
...
export class ApplicationShellImpl implements ApplicationShell {
  attach(host: HTMLElement): void {
    host.textContent = "Hello, Cell.js.";
  }
}
```

我们可以看到 attch 方法接为一个类型为`HTMLElement`的参数 host，并调用`host.textContent = 'Hello, Cell.js.'`赋值实现了一个前端的渲染动作。
...
IoC 是面向对象编程的一种设计原则，Cell.js 中主要依靠一系列的注解器(Annotation)来组织代码使之匹配这种设计原则。来看一段示例代码：
...
- 容器实现：[common/container](https://github.com/cellbang/cell/tree/master/packages/core/src/common/container)
- IOC 相关的注解： [common/annotation](https://github.com/cellbang/cell/tree/master/packages/core/src/common/annotation)
...
- 定义 AOP 的 interface 和相关实现: [common/aop](https://github.com/cellbang/cell/tree/master/packages/core/src/common/aop)
- AOP 相关的注解: [common/annotation](https://github.com/cellbang/cell/tree/master/packages/core/src/common/annotation)
- autoBind 中对 AOP 类进行包装: [common/container/auto-bind.ts](https://github.com/cellbang/cell/blob/master/packages/core/src/common/container/auto-bind.ts)

## 日志

Cell.js 核心提供了对日志的封装，代码在[common/logger/logger.ts](https://github.com/cellbang/cell/blob/master/packages/core/src/common/logger/logger.ts):
...
核心包内默认的日志功能较为精简，如果有更复杂的需求可以自行定义 Logger 来扩展，或者使用`@celljs/logger`包。
