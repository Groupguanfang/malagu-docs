...
title: 数据格式
icon: i-ph-dna
description: Cell.js 提供了一种开箱即用的方式，帮助使用者自动处理数据格式。
---

为了方便前端对后端接口返回值做统一处理，前端可能希望后端返回的结果格式是统一的，如果后端在每个接口处都做数据格式处理，显得繁琐而重复。或者基于 Cell.js 框架提供的 ErrorHandler 和 View 接口做统一拦截处理。
