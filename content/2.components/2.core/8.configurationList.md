...
把 mode 当开关来用，比如框架在集成 React 前端框架的时候，集成组件`@malagu/react`中提供了一个 malagu-cdn.yml 的模式配置文件，默认该模式文件不会被加载，当我们指定 mode 为 cdn 时，则开起了从 cdn 中加载 React 框架代码的能力。
