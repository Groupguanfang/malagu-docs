---
title: 配置文件
icon: i-fluent-document-yml-16-regular
description: Cell.js使用cell-[mode].yml文件来配置应用程序。
---

Cell.js 框架很多地方使用到了 EL 表达式功能，EL 表达式使用了开源库<a target="_blank" href="https://www.npmjs.com/package/jexl">jexl</a>实现。EL 表达式让组件属性的定义与使用变得极度的灵活。

组件属性默认在组件项目的根目录下加载 `cell.yml` 属性文件，当 `cell.yml` 配置了属性 `mode: test` ，则尝试加载根目录的 `cell-test.yml` 属性文件，规则是： `cell-[mode].yml` 。

配置文件分类如下：

- 公共属性文件：`cell.yml`
- 模式属性文件：`cell-[mode].yml`
...
```yml [cell.yml]
# 指定单个模式，该模式下，会尝试加载所有组件的 cell.yml 和 cell-prod.yml
mode: prod

#指定多模式，该模式下，会尝试加载所有组件的 cell.yml、cell-prod1.yml 和 cell-prod2.yml
mode: [prod1, prod2]
```
...
#指定多模式，该模式下，会尝试加载所有组件的 cell.yml、cell-prod1.yml 和 cell-prod2
cell deploy -m prod1,prod2
```
...
在 Cell.js 框架中，使用 Yaml 文件配置组件属性。为了让属性普通值与 EL 表达式区别开来，Cell.js 使用 `${}`和`${{}}`包裹 EL 表达式。一个属性值可以包含多个 EL 表达式和字符串的组合，并且 EL 表达式还可以嵌套使用。
