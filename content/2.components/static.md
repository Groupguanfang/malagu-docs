...
description: Cell.js 框架通过与serve-static库结合实现静态资源的访问，让您的应用程序更加完善。
---

我们开发前端的时候，往往会采用前后端分离方案，前端编译构建的静态资源文件一般托管到一个静态资源服务上，然后通过 nginx 做反向代理。随着云计算的流行，也有越来越多的开发者将静态资源文件托管到对象存储服务或 Serverless 平台中。

Cell.js 框架也提供了静态资源服务组件 `@celljs/serve-static`。如果我们使用 Cell.js 框架开发前端应用，通过该组件，我们能快速地搭建静态资源服务，并且是零配置的，开箱即用。由于 Cell.js 框架是 Serverless Frist，一行代码都无需改动，就可以一键把静态资源服务部署到 Serverless 平台上运行。

## 前后端一体化应用

Cell.js 框架可以开发任意类型的应用，我们可以将静态资源服务独立部署到云服务器、Serverless 平台上，也可以将静态资源服务与后端 API 服务一体化部署到云服务器、Serverless 平台上。Cell.js 框架会处理好静态资源服务与后端 API 服务之间的融合。

以阿里云 Serverless 平台函数计算服务为例，如果我们采用 Cell.js 框架的前后端一体化方式开发应用，Cell.js 框架在部署到函数计算的时候，将前后端一体化应用部署为函数计算的一个函数。您可能会有疑问，如果该前后端一体化应用很大，部署为一个函数，虽然管理起来方便，但是会增加函数的运行负担。确实如此，如果我们的应用不是很大，采用一个应用对应一个函数，不会有任何负担，Cell.js 会通过合理的手段优化，让函数的冷启动时间足够短，几乎察觉不到冷启动。如果我们的应用很大，我们确实不能这么做，不管我们是否采用 Serverless 这个方案，应用足够大的时候，我们都不应该开发这样一个巨石应用，我们应该将这样的巨石应用合理拆分成许多微应用，每一个微应用对应着函数计算中的一个函数是没有任何问题的。
