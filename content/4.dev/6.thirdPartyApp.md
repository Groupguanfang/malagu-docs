...
description: 将 Cell.js 核心组件集成第三方应用程序，复用 Cell.js 提供的 IoC 能力，在这里我们提供了一些集成案例，从而举一反三。
---

Cell.js 框架核心组件是提供了 IoC 能力的基础组件。我们使用 Cell.js 命令行可以快速初始化一个 Cell.js 应用，在 Cell.js 应用中默认集成了核心组件，并且基于 Cell.js 命令行工具开箱即用。也有不少开发者咨询如何将 Cell.js 核心组件集成第三方应用程序，复用 Cell.js 提供的 IoC 能力，在这里我们提供了一些集成案例，方便大家参考，从而举一反三。

Cell.js 框架核心组件提供了 ContainerFactory 和 ApplicationFactory 工厂类，都可以快速创建一个 IoC 容器，前者是后者的功能子集。ApplicationFactory 工厂类除了创建 IoC 容器以外，还提供了应用生命周期管理。大部分情况推荐使用 ApplicationFactory 工厂类。
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // 在 Cell.js 应用中，一般是通过 cell.yml 配置文件提供配置信息，而在第三方集成方案中，可以通过第一个参数提供
