---
title: CI/CD
icon: i-clarity-ci-cd-line
description: Cell.js 框架提供了一系列开箱即用的模板，在模板中，默认提供了基于 GitHub Actions 实现 CI/CD 脚本，我们只需要在平台中注入相应的秘钥即可使用。
---

在一个多人的开发团队，为了提升集成和部署的效率，同时也为了一些秘钥的安全性，我们需要提供持续集成和持续部署（CI/CD）能力。供持续集成和持续部署（CI/CD）的关键在于我们如何将秘钥注入到 CI/CD 的环境中，已经框架如何正确的获取该秘钥。注入秘钥一般由相关的平台来做，比如 GitHub；而获取秘钥，一般是由应用本身来处理。

## 框架默认提供 CI/CD 脚本

Cell.js 框架提供了一系列开箱即用的模板，在模板中，默认提供了基于 GitHub Actions 实现 CI/CD 脚本，我们只需要在平台中注入相应的秘钥即可使用。脚本如下：
...
GitHub 平台会将秘钥，通过环境变量注入到 CI/CD 运行环境中，在 Cell.js 框架中，我们可以通过属性配置文件引用当前环境中的环境变量，如下所示：
...
如果需要部署到云厂商的 Serverless 平台上，需要将相关的 AKSK 相信配置为秘钥，并注入到 CI/CD 环境中，配置一下秘钥即可：
...
1. 在 Coding `持续集成-构建计划` 页面中，点击右上角 `创建构建计划`

![创建构建计划.svg](/images/coding_1.webp)

2. 在 `选择构建计划模版` 页面中，选择 `自定义构建过程`

![创建构建计划.svg](/images/coding_2.webp)

3. 填写自定义构建过程中的 `构建计划名称` ，选择代码仓库，配置来源选择 `使用静态的Jenkinsfile` ,（也可以将配置信息放入代码仓库，使用代码库中的 Jenkinsfile）,

![创建构建计划.svg](/images/coding_3.webp)

4. 在 `流程配置-文本编辑器` 中，填入上述脚本，在 `环境变量` 中，填入脚本中所需的参数,（即云平台部署中所需的 env 参数，可参考云平台文档中的配置说明）,点击保存。

![创建构建计划.svg](/images/coding_4.webp)

5. 返回 `构建计划` ，点击 `立即构建` ，此时已经可以成功构建，前往云平台控制台，可发现函数已经成功部署。

![创建构建计划.svg](/images/coding_5.webp)
