...
Malagu 框架很多地方的实现都采用上面的技巧，把本来需要很多个`if/else`才能完成的功能，拆解成统一接口的许多实现，最后这些实现都以同一个接口为 ID 托管到容器中。这种实现模式，可以将冗长、逻辑关系复杂的代码实现拆解成功能单一的实现，代码可读性更高，也便于使用同一接口追加新的接口实现。追加新的接口实现，只需要按照同一个接口作为 ID 托管到容器即可。比如我们可以追加一个新的中间件实现，代码如下：
