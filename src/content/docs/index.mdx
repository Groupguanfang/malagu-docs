...
<CardGrid stagger>
  <Card title="Serverless First" icon="seti:html">
    基于 TypeScript，坚信 Serverless 是未来，适配`阿里云`、`腾讯云`、`AWS Lambda`、`Vercel`等平台。
  </Card>
  <Card title="组件化 + 模块化" icon="puzzle">
    在 Malagu 的世界里：“一切皆组件”。多个组件组合成一个模块，一个或多个模块组合成一个真实的应用。
  </Card>
  <Card title="平台无关" icon="seti:ionic">
    适配任意的平台和基础框架，是一个平台或基础框架无关的上层框架。
  </Card>
  <Card title="IOC + AOP" icon="seti:ionic">
    基于依赖注入和切面编程，抽象化解藕业务逻辑，提高代码的可维护性。
  </Card>
  <Card title="渐进式" icon="seti:ionic">
    适配传统的 Web 应用、微服务、Serverless 应用，支持渐进式的应用开发。
  </Card>
  <Card title="开箱即用" icon="seti:ionic">
    提供了一整套的开发工具和组件，开发者可以自由地组合组件，快速地搭建应用，直接撰写业务代码。
  </Card>
</CardGrid>
...
- 文档整理：[Zero](http://github.com/groupguanfang)
- 仓库地址：[Malagu Docs](https://github.com/Groupguanfang/malagu-docs)
